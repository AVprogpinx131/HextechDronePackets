...
func GetMovementsByUser(userID int) ([]models.DroneMovement, error) {
    query := `
    SELECT dm.id, dm.mac, dm.event_type, dm.territory_id, t.name AS territory_name, dm.timestamp 
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&m.ID, &m.MAC, &m.EventType, &m.TerritoryId, &m.TerritoryName, &m.Timestamp)
        if err != nil {
